...
function sanitizeFileName(fileName: string) {
  ...
  return `${base || 'photo'}${extension.replace(/[^.a-z0-9]/g, '')}`
}
...
Deno.serve(async (request) => {
  ...
  if (
    ...
    return json({ error: 'Gallery upload is not configured.' }, 500)
  }
  ...
  if (canEdit !== true) {
    return json({ error: 'You do not have permission to upload gallery images.' }, 403)
  }
  ...
  const galleryId = payload.galleryId?.trim()
  const fileName = payload.fileName?.trim()
  const contentType = payload.contentType?.trim().toLowerCase()
  const fileSize = payload.fileSize

  if (!galleryId || !isUuid(galleryId)) {
    return json({ error: 'Invalid galleryId.' }, 400)
  }
  ...
  // Make sure the editor is uploading into an existing gallery they can access.
  ...
  if (galleryError) {
    console.error('Gallery lookup failed', galleryError)
    return json({ error: 'Unable to verify gallery.' }, 500)
  }
  ...
  const objectKey = `galleries/${galleryId}/${crypto.randomUUID()}-${safeName}`
